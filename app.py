...
def confg():
    engine= create_engine('postgresql://{postgres}:{password}@localhost:5432/tourism_db')
    Base = automap_base()
...
def handler_arrival_data():
    if request.method == 'GET':
        ...
        li=[]
        for i in arrival:
            json_type = OrderedDict()
            json_type['type'] = 'Feature'
            json_type['geometry'] = {
                    'type': 'Point',
            ...
            li.append(json_type)
        json_type = OrderedDict()
        json_type['type'] = 'FeatureCollection'
        json_type['features'] = li
        return json_type
...
# @app.route("/")
# def index():
#     arrivals,revenue,session=confg()
